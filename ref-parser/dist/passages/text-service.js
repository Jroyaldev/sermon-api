...
import { getDefaultTranslation, resolveTranslation, } from "./translation-metadata";
const BOOK_OSIS_BY_NUMBER = [
    "",
    "Gen",
    "Exod",
    "Lev",
...
const BOOK_ORDER = new Map(BOOK_OSIS_BY_NUMBER.map((osis, index) => [osis, index]));
...
export async function fetchPassageText(args) {
...
export async function fetchBatchPassageText(args) {
    const translation = resolveTranslation(args.translation) ?? getDefaultTranslation();
    const bible = await loadBible(translation);
    if (!bible) {
...
export async function parseBibleFromXml(translation) {
        ...
        for (const bookNode of booksArray) {
            ...
            for (const chapterNode of chapterNodes) {
                if (!chapterNode || typeof chapterNode !== "object")
                    continue;
                const chapterRecord = chapterNode;
                const cnumber = toInteger(pickProperty(chapterRecord, ["cnumber", "cnum", "number", "n"]));
                if (!Number.isInteger(cnumber) || cnumber === undefined)
                    continue;
                ...
                for (const verseNode of verseNodes) {
                    if (!verseNode || (typeof verseNode !== "string" && typeof verseNode !== "object"))
                        continue;
                    const verseRecord = typeof verseNode === "object" ? verseNode : undefined;
                    const vnumber = verseRecord ? toInteger(pickProperty(verseRecord, ["vnumber", "vnum", "number", "n"])) : undefined;
                    if (!Number.isInteger(vnumber) || vnumber === undefined)
                        continue;
...
async function resolveBibleData(translation) {
...
async function loadCompiledBible(translation) {
...
function buildPassageResponse(bible, osis, includeContext) {
...
function expandOsis(bible, osis) {
    const result = [];
...
function parseAllPointers(bible, expression, fallback) {
    ...
    if (!chapterData.verses.has(verse))
        return [];
    return [{ bookOsis, chapter, verse }];
}
function parseFirstPointer(bible, expression, fallback) {
    const pointers = parseAllPointers(bible, expression, fallback);
    return pointers[0];
}
function parseLastPointer(bible, expression, fallback) {
    const pointers = parseAllPointers(bible, expression, fallback);
    return pointers.at(-1);
}
function collectRange(bible, start, end) {
    const range = [];
    let current = start;
    while (current) {
        range.push(current);
        if (comparePointers(current, end) >= 0)
            break;
        current = getNextPointer(bible, current);
    }
    return range;
}
function getNextPointer(bible, pointer) {
...
function getPreviousPointer(bible, pointer) {
    const book = bible.books.get(pointer.bookOsis);
    if (!book)
        return undefined;
    const chapter = book.chapters.get(pointer.chapter);
...
function extractComponents(expression) {
...
function getVerseData(bible, pointer) {
    const book = bible.books.get(pointer.bookOsis);
    const chapter = book?.chapters.get(pointer.chapter);
    return chapter?.verses.get(pointer.verse);
}
function formatContextVerse(bible, pointer) {
    const verse = getVerseData(bible, pointer);
    if (!verse)
        return undefined;
    return `${verse.reference} ${verse.text}`.trim();
}
function findAdjacentBook(currentOsis, step) {
...
function extractVerseText(node) {
...
function serializeBibleData(bible) {
    const books = {};
...
function deserializeBibleData(payload) {
...
function pickProperty(record, keys) {
...
function pickString(record, keys) {
    const candidate = pickProperty(record, keys);
    return typeof candidate === "string" ? candidate.trim() : undefined;
}
function resolveBookOsis(record, fallbackNumber) {
